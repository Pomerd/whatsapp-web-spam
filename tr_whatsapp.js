...
// @version      2.0
// @description  WhatsApp Web için stabil otomatik mesaj gönderici
// @author       Pomerd 
...
(function() {
    ...
    function createUI() {
        ...
        countLabel.textContent = 'Gönderim Sayısı:';
        ...
        delayLabel.textContent = 'Gecikme (ms):';
        ...
        startBtn.addEventListener('click', () => {
            ...
            if (!text || count < 1) {
                updateStatus('⚠️ Mesaj veya sayı geçersiz!', 'error');
                return;
            }
            ...
            updateStatus(`✔️ ${count} mesaj sıraya alındı`);
    ...
    function sendQueue() {
        if (queue.length === 0) {
            isSending = false;
            updateStatus('✅ Tüm mesajlar gönderildi', 'success');
            return;
        }
        isSending = true;
        const text = queue.shift();
        sendMessage(text);
        updateStatus(`${queue.length} mesaj kaldı...`);
